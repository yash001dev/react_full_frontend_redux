...
const useUpdateForm=(validateInfo,getChemistData,deleteData,updateData,chemistData)=>{
    ...
    const [isSubmitting,setIsSubmitting]=useState(false)
     ...
     const deleteHandleClickOpen=(data)=>{
      ...
      const handleDelete=()=>{
        ...
        console.log("DELETED ID:",deleteId.workplace_id);
        axios.delete(`http://localhost:3001/api/workplace/delete/${deleteId.workplace_id}`);
        deleteData(deleteId.workplace_id);
        setDeleteOpen(false);
      }
    ...
    useEffect(()=>{
        if(Object.keys(errors).length===0 && isSubmitting){
            ...
            axios.put('http://localhost:3001/api/workplace/update',values);
            setValue({
                workplace:'',                
